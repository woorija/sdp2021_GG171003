...
public class CustomInputManager : MonoSingleton<CustomInputManager>
{
    ...
    public KeyInfor[] keyinfor { get; private set; } = new KeyInfor[8];
    ...
    {
        if((int)_e.keyCode >= 48 && (int)_e.keyCode <= 57 || (int)_e.keyCode >= 97 && (int)_e.keyCode <= 122 || (int)_e.keyCode == 32 || (int)_e.keyCode >= 273 && (int)_e.keyCode <= 276) // 숫자와 영문,스페이스,방향키
        {
            return true;
        }
        else
    ...
    public bool GetKey(int _index)
    {
        if (keyinfor[_index]._key == 0) return false;
        if (!ModifiersCheck(_index)) return false;
        if (Input.GetKey((KeyCode)keyinfor[_index]._key)) return true;
        return false;
    }
}
